...
import { RASTER_TOOL_NAMES, registerRasterTools } from "./rasterTools.js";
...
export function registerTools(server: McpServer, session: CadSession): void {
  ...
  server.registerTool(
    ...
    async () => {
      try {
        const status = await session.geometryEngine.getBackendStatus();
        return mcpJson({ success: true, data: status });
      } catch (err) {
        return toolError(err);
      }
    },
  ...
  registerRasterTools(server, session);
